...
export function TrendChart({ weeks }: { weeks: Week[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full" role="img"
      ...
      {[3, 4].map(s => (
        <g key={s}>
          <line x1={LEFT} x2={W} y1={y(s)} y2={y(s)} className="stroke-base-300" strokeDasharray="2 3" />
          <text x={0} y={y(s) + 4} className="fill-base-content/50 text-[10px]">{s}</text>
        </g>
      ))}
      <line x1={LEFT} x2={W} y1={BOTTOM} y2={BOTTOM} className="stroke-base-300" />
      {weeks.map((w, i) => {
        ...
        return (
          <g key={w.start}>
            {w.avg === null ? (
              ...
              <rect x={x} y={y(w.avg)} width={bar} height={Math.max(BOTTOM - y(w.avg), 1)} rx={2}
            ...
            <text x={cx} y={BOTTOM + 16} textAnchor="middle" className="fill-base-content/60 text-[10px]">
              {dayMonth(w.start)}
            </text>
          </g>
        );
